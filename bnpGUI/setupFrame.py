...
class setupFrame():
    ...
    def choose_folder(self):
        self.h5_folder = tk.filedialog.askdirectory(initialdir = '/mnt/micdata1/bnp/2021-3/')
        self.selectfolder.set(self.h5_folder)
        ext = '.h5'
        self.files_combobox['values'] = [i for i in os.listdir(self.h5_folder) if i[-len(ext):] == ext]
    ...
    def load_scan(self, *args):
        ...
        if (ctime - fmtime) >= t_diff:
            try:
                ...
                dets = self.h5['/MAPS/channel_names'][:].astype(str).tolist() + \
                        self.h5['/MAPS/scaler_names'][:].astype(str).tolist()
                elmScalers = np.vstack((self.h5['/MAPS/XRF_roi'][:], 
                                        self.h5['/MAPS/scalers'][:]))
                ...
                self.file_z = float(pvval[pvlist.index(self.pvComm.pvs['z_value_Act'].pv.pvname)])
                self.file_theta = float(pvval[pvlist.index(self.pvComm.pvs['sm_rot_Act'].pv.pvname)])
                self.Image2D = self.Axe2D.imshow(elmScalers[i_det], 
                                                 aspect='equal', interpolation='nearest', 
                                                 cmap="inferno", 
                                                 norm=colors.LogNorm() if self.log_button.config('relief')[-1] == 'sunken' else colors.Normalize() )
                self.Canvas2D.draw()
    ...
    def Draw_Rectangle(self, xmin, xmax, ymin, ymax, color, lw = 2, animated = False):
        ...
        alpha = 1, edgecolor = 'w', fill = False, linewidth= lw, animated = animated)
...
#        return Dot
    ...
    def Canvas2D_Button_Pressed(self, event):
        self.xstart, self.ystart = list(map(lambda x: int(round(x, 0)), (self.Display2Data(self.Axe2D, event.x, event.y))))
        if event.button == 1:
            self.xycorr.set('x, y: (%.2f, %.2f)'%(self.x[self.xstart], self.y[self.ystart]))
        if (event.button == 1) & (self.insertType.get() =='XY Center'):
            ...
            self.Canvas2D_Button_Release_Event = self.Canvas2D.mpl_connect('button_release_event', self.Canvas2D_Button_Released)

    def Canvas2D_Button_Released(self, event):
        self.xend, self.yend = list(map(lambda x: int(round(x, 0)), (self.Display2Data(self.Axe2D, event.x, event.y))))
       
        self.Canvas2D.mpl_disconnect(self.Canvas2D_Button_Release_Event)
        self.Canvas2D_Button_Press_Event = self.Canvas2D.mpl_connect('button_press_event', self.Canvas2D_Button_Pressed)
    ...
    def Canvas2D_Mouser_Hover(self, event):
        ...
        try:
            ...
            return
        if (self.yend < dim_y-0.5) and (self.yend > -.5) and (self.xend > -0.5) and (self.xend < dim_x-0.5):
            if self.Rectangle_Drawing:
                if self.non_animated_background != None:
                    ...
                    ymax = int(max(self.yend, self.ystart))
                    ymin = int(min(self.yend, self.ystart))
                    self.Rectangle = self.Draw_Rectangle(xmin, xmax, ymin, ymax, 'w', animated = True)
                    self.Canvas2D.draw()
                    self.non_animated_background = self.Canvas2D.copy_from_bbox(self.Axe2D.bbox)
    ...
    def addToScanBtn(self, func):
        add_scan_btn = tk.Button(self.setupfrm, text='Add to scan list', 
                                 command=func, width = 50)
    ...
    def updatexyz0(self):
        if (self.pvComm.getSMAngle() ** 2) < 1e-3:
            ...
            self.updatexyz_msg.set('x-, y-, z- theta0 updated')
    ...
    def __init__(self, tabControl):
        ...
        mdafolder_button = tk.Button(self.setupfrm, text='Open Folder', command=self.choose_folder)
        ...
        self.files_combobox = ttk.Combobox(self.setupfrm, textvariable = self.scfile_sv, state = 'readonly', width = 22)
...
#        self.scannum_entry.grid(column=1, row = 1, padx=(5,5), pady=(5,5))
        ...
        loadscan_button = tk.Button(self.setupfrm, text='Update', command=self.updateFileList)
        loadscan_button.grid(column=2, row = 1, padx=(5,5), pady=(5,5))
        detector_sv = tk.StringVar()
        self.detector_combobox = ttk.Combobox(self.setupfrm, textvariable=detector_sv, state='readonly', width=20)
        self.detector_combobox.grid(column=3, row=1, padx=(5,5), pady=(5,5))
        self.detector_combobox.bind('<<ComboboxSelected>>', self.plot_data)

        self.log_button = tk.Button(self.setupfrm, text='Log', command=self.logscale_changed)
        ...
        self.Canvas2D.get_tk_widget().grid(column=0, columnspan=10, row=2, rowspan=50, padx=(10,5), pady=(5,5), sticky='W')

        self.Canvas2D_Mouse_Hover_Event = self.Canvas2D.mpl_connect("motion_notify_event", self.Canvas2D_Mouser_Hover)
        self.Canvas2D_Button_Press_Event = self.Canvas2D.mpl_connect("button_press_event", self.Canvas2D_Button_Pressed)
        self.Rectangle_Drawing = False
        ...
        self.row = 2
        self.col = 10
        self.scantype_txt = tk.Label(self.setupfrm, text = '1. Choose a type of scan below:')
        self.scantype_txt.grid(row = self.row, column=self.col, padx=(0,400), columnspan = 8)
        ...
        for i, s in enumerate(scantype):
            a = ttk.Radiobutton(master = self.setupfrm, text = s, 
                                variable=self.scanType, value=s)
            a.grid(row = self.row, column = self.col+i, padx=padx[i])
            
        self.row += 1    
        self.inserttype_txt = tk.Label(self.setupfrm, text = '2. Choose a method to insert scan parameters:')
        self.inserttype_txt.grid(row = self.row, column = self.col, padx=(0,320), columnspan = 8)
        ...
        for i, s in enumerate(inserttype):
            a = ttk.Radiobutton(master = self.setupfrm, text = s, 
                                variable=self.insertType, value=s)
            a.grid(row = self.row, column = self.col+i, padx=padx[i])
        ...
        self.col = 10
        ncol = 3   
        self.inputs_labels = [['x_theta0', 'y_theta0', 'z_theta0', 'x_scan', 
                               'y_scan', 'z_scan', 'dwell', 'width', 'w_step', 
                               'target_theta', 'height', 'h_step'],
                            ['theta_min', 'theta_max', 'theta_inc',
                          'elm', 'width_fine', 'w_step_fine',
                         'n_clusters', 'height_fine','h_step_fine',
                         'sel_cluster', 'dwell_fine',
                         'use_mask', 'mask_elm']]
#        corr = self.pvComm.getXYZcenter()
        ...
        for i, s in enumerate(self.inputs_labels[0]):
            if i% ncol == 0: self.row += 1
            sv = tk.StringVar()
            sv.trace('w', lambda name, index, mode, sv = sv: self.calcTime())
            tk.Label(self.setupfrm, text = s+':').grid(row = self.row, column = self.col + i%ncol, sticky='W', columnspan=2)
            a = tk.Entry(self.setupfrm, width = 10, textvariable=sv, validate = 'all', validatecommand=(vcmd, '%P'))
...
#            a.insert(0,'0')
        ...
        smp_entry.grid(row = self.row, column = self.col, columnspan = 2, sticky = 'w', padx=(100,0))
        ...
        bda_entry = tk.Entry(self.setupfrm, textvariable = self.bda, 
                             validate = 'all', validatecommand=(vcmd, '%P'))
        
        # TO DO: get bda position from save state instead from motor
        self.bda.set('%.2f'%(self.pvComm.getBDAx()))
        bda_entry.grid(row = self.row, column = self.col + 2, sticky = 'w', padx=(25,0),
                       columnspan = 2)
      
        self.row += 1
        updatexyz0_btn = tk.Button(self.setupfrm, text = 'theta 0 (from motor)', command = self.updatexyz0)
        updatexyz0_btn.grid(row = self.row, column = self.col, columnspan = 2)
        coordtform_btn = tk.Button(self.setupfrm, text = 'Convert xyz-theta0 to xyz-scan', command = self.coordtform)
        coordtform_btn.grid(row = self.row, column = self.col + 1, columnspan = 2)
        ...
        self.calctime.set('0')
        self.calctime_out = tk.Label(self.setupfrm, width = 10, textvariable = self.calctime)
        ...
        self.updatexyz_label.grid(row = self.row, column = self.col, sticky = 'w', columnspan = 3)
        
        self.row += 4
        self.inserttype_txt = tk.Label(self.setupfrm, text = 'Additional scan parameters for tomography')
        self.inserttype_txt.grid(row = self.row, column=self.col, padx=(0,0), columnspan = 8)
        for i, s in enumerate(self.inputs_labels[1]):
            ...
            if (s == 'elm') | (s == 'mask_elm'):
                ...
                a = tk.Entry(self.setupfrm, width = 10, validate = 'all', validatecommand=(vcmd, '%P'))
                a.insert(0,'0')
            a.grid(row = self.row, column = self.col + i%ncol, padx=(70,0))
            self.scanParms.update({s:a})
        ...
        self.pardir.set(self.pvComm.getDir())
        scdir_txt = tk.Button(self.setupfrm, text = 'Current data save directory:',
                              command=self.getUserDir)
